...
class MyModel(object):
    ...
    def __init__(self,
        ...
        with tf.variable_scope('embedding_layer'):
            embedding_matrix_char = tf.get_variable("embedding_matrix_char", [vocab_size_char, embedding_dim],
                                                    dtype=tf.float32)
            embedding_matrix_word = tf.get_variable("embedding_matrix_word", [vocab_size_word, embedding_dim],
                                                    dtype=tf.float32)
        ...
        with tf.variable_scope('loss'):
            ...
            loss_attr = tf.reduce_sum(loss_attr * masks_attr, axis=-1) / (
                        tf.reduce_sum(masks_attr, axis=-1) + 1e-5)  # B
