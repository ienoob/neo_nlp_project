...
class LatticeLSTM(nn.Module):
    ...
    def forward(self, input, skip_input_list, hidden=None, volatile_flag=False):
        """
...
class LatticeLSTMModel(nn.Module):

    def __init__(self, char_size, char_embed, lstm_hidden, label_alphabet_size, gaz_dropout=0.3, gaz_alphabet_size=10, gaz_emb_dim=64, pretrain_gaz_embedding=None, HP_fix_gaz_emb=True):
        super(LatticeLSTMModel, self).__init__()
        self.gpu = False
        self.char_embed = nn.Embedding(char_size, char_embed)
        self.drop = nn.Dropout(0.5)
        self.bichar_embed = None
        lstm_input = char_embed
        self.lattice_lstm = LatticeLSTM(lstm_input, lstm_hidden, gaz_dropout, gaz_alphabet_size, gaz_emb_dim, pretrain_gaz_embedding, True, HP_fix_gaz_emb, self.gpu)
        HP_hidden_dim = 64
        # label_alphabet_size = 64
        self.hidden2tag = nn.Linear(HP_hidden_dim, label_alphabet_size+2)
        self.droplstm = nn.Dropout(gaz_dropout)

        self.crf = CRF(label_alphabet_size, self.gpu)

    def neg_log_likelihood_loss(self, gaz_list, char_inputs, batch_label):
        mask = char_inputs.ne(0)
        char_embed = self.char_embed(char_inputs)
        char_embed = self.drop(char_embed)

        hidden = None
        lstm_out, hidden = self.lattice_lstm(char_embed, gaz_list, hidden)
        tags = self.droplstm(lstm_out)
        tags = self.hidden2tag(tags)
    ...
    def forward(self, gaz_list, char_inputs):
        mask = char_inputs.ne(0)
        char_embed = self.char_embed(char_inputs)
        # char_embed = self.drop(char_embed)
