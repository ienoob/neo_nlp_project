...
def train_step(input_xx, input_xy, input_yy):
    ...
    with tf.GradientTape() as tape:
        _, encoder_state = encoder(input_xx)
        decoder_out, _ = decoder(input_xy, input_state=encoder_state)

        loss_v = loss_func(input_yy, decoder_out)
...
def predict(input_s_list):
    ...
    _, encoder_state = encoder(input_s_id)

    input_decoder_state = encoder_state
    input_xy = [[label2id["<start>"]]*len(input_s_id)]
    ...
    output_label = [[id2label[o] for o in output_id][:len(input_s_list[i])] for i, output_id in
