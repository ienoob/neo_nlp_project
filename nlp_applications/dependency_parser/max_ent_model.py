...
class maxEnt:
    ...
    def __init__(self, trainDataList, trainLabelList, testDataList, testLabelList):
        ...
        self.label_value = {t for t in trainLabelList}
        print(self.label_value)
    ...
    def maxEntropyTrain(self, iter = 500):
        ...
        for i in range(iter):
            ...
            iterEnd = time.time()
            #打印运行时长信息
            print('iter:%d:%d, time:%d'%(i, iter, iterStart - iterEnd), " target value {}".format(np.sum(Epxy)))
