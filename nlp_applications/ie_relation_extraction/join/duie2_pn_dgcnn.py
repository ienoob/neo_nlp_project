...
from nlp_applications.ie_relation_extraction.join.pn_dgcnn import DgCnnPN
...
class DataIterator(object):
    ...
    def single_doc_processor(self, doc: Document):
        ...
        evaluation_label = []
        word_encode_id = []
        pos_encode_id = [i+1 for i in range(len(encoding))]
    ...
    def padding_batch_data(self, input_batch_data):
        ...
        for data in input_batch_data:
            ...
            batch_pos_encode_id.append(data["pos_encode_id"])
        ...
        return {
            ...
            "pos_encode_id": tf.keras.preprocessing.sequence.pad_sequences(batch_pos_encode_id, padding="post"),
...
def main():
    ...
    parser.add_argument("--char_size", type=int, default=vocab_size, required=False)
    parser.add_argument("--char_embed_size", type=int, default=64, required=False)
    ...
    def train_step(input_x, input_pos_x, input_word_x, input_sub_loc, input_sub_label, input_po_label):
        with tf.GradientTape() as tape:
            sub_logits, po_logits, data_mask = pm_model(input_x,
                                                        inputs_word=input_word_x,
                                                        inputs_position=input_pos_x,
                                                        input_sub_loc=input_sub_loc, training=True)
